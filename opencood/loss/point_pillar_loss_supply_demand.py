...
class PointPillarLossSupplyDemand(nn.Module):
    def __init__(self, args):
        super(PointPillarLossSupplyDemand, self).__init__()
        self.pos_cls_weight = args['pos_cls_weight']

        self.cls = args['cls']
        self.reg = args['reg']
        self.supply_sum_loss = args['supply_sum_loss']
        self.demand_sum_loss = args['demand_sum_loss']
        self.demand_mse_loss = args['demand_mse_loss']
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        weighted_supply_sum_loss = self.supply_sum_loss['weight'] * output_dict['supply_sum_loss']
        weighted_demand_sum_loss = self.demand_sum_loss['weight'] * output_dict['demand_sum_loss']
        weighted_demand_mse_loss = self.demand_mse_loss['weight'] * output_dict['demand_mse_loss']
        total_loss += weighted_supply_sum_loss + weighted_demand_sum_loss + weighted_demand_mse_loss
        self.loss_dict.update({'supply_sum_loss': weighted_supply_sum_loss.item(),
                                 'demand_sum_loss': weighted_demand_sum_loss.item(),
                                 'demand_mse_loss': weighted_demand_mse_loss.item()})
    ...
    def logging(self, epoch, batch_id, batch_len, writer=None, pbar=None, suffix=""):
        ...
        supply_sum_loss = self.loss_dict.get('supply_sum_loss', 0)
        demand_sum_loss = self.loss_dict.get('demand_sum_loss', 0)
        demand_mse_loss = self.loss_dict.get('demand_mse_loss', 0)

        if pbar is not None:
            pbar.set_description("[e %d][%d/%d]%s | L: %.4f, Conf: %.4f"
                ", Loc: %.4f, Dir: %.4f, SS: %.4f, DS: %.4f, DM: %.4f" % (
                    epoch, batch_id + 1, batch_len, suffix,
                    total_loss, cls_loss, reg_loss, dir_loss, supply_sum_loss, demand_sum_loss, demand_mse_loss))
        else:
        ...
        if not writer is None:
            ...
            writer.add_scalar('Iou_loss'+suffix, iou_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Supply_sum_loss'+suffix, supply_sum_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Demand_sum_loss'+suffix, demand_sum_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Demand_mse_loss'+suffix, demand_mse_loss,
                            epoch*batch_len + batch_id)
