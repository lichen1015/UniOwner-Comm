...
def inference_early_fusion(batch_data, model, dataset):
    ...
    if isinstance(output_dict['ego'], dict) and ('comm_rate' in output_dict['ego']):
        return_dict['comm_rate'] = output_dict['ego']['comm_rate']
        return_dict['comm_kbps_per_frame'] = output_dict['ego']['comm_kbps_per_frame']
